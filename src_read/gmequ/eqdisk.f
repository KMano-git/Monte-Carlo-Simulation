...
!ik   integer i, imax, j, lenx, nk, mj
      integer i, imax, j, mj, n
...
!-----
!
!     To be consisit with the valuable order of dat.eqil
      read(nft,'(2i5)') nr, nz
      read(nft,cfmtR) rmin, rmax, zmin, zmax, dr0
      read(nft,cfmtR) dz0
      read(nft,cfmtR) rbt0
      ...
      do i = 1, nr*nz, 5
        read(nft,cfmtR)
     >    psi(i),psi(i+1),psi(i+2),psi(i+3),psi(i+4)
        if(i+9 >= nr*nz .and. i+4 <= nr*nz) then
          read(nft,cfmtR) (psi(n),n=i+5,nr*nz)
          exit
        endif
      enddo
      close(nft)
