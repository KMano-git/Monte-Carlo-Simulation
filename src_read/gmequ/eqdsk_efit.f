...
!::local common
!xx      integer ndr, ndz
!xx      parameter  (ndr = 129, ndz = 129)
!xx      real*8, dimension(ndr) :: rg
!xx      real*8, dimension(ndz) :: zg
!xx      real*8, dimension(ndr*ndz) :: psi
!xx      integer :: nr, nz
!xx      real*8  :: dr, dz
!xx      real*8  :: rbt0
!xx      real*8  :: raxs, zaxs, paxs
!xx      real*8  :: rsep, zsep, psep
!
!
!::local variables
! modified 1/1 lines replace all include files with module files by kamata 2021/08/18
!ik   integer  :: i, j, k, nls, nle, nbf, ierr
      integer  :: i, j, k, ierr, itag, ityp, ierr_io
      integer  :: ishot, it
      real*8   :: eqtime
...
!::EFIT-data
!-----------------------------------------------------------------------
      read(nft,2000) (case(i),i=1,5), idum, nw, nh
      nf = nw
!
      xdbnm = case(1)
      xdate = case(2)
      read(case(3),'(i8)') ishot
      read(case(4)(1:len(case(4))-2),'(i6)') it
...
!
      ...
      read(nft,2020,iostat=ierr_io) (qpsi(i),i=1,nf)
!
      if(ierr_io.eq.0)then
      read(nft,2022)  nbbbs,limitr
      write(n6,'(2x,"nbbbs  =",i4," <= ",i4,"  limitr =",i4," <= ",
     >  i4)')  nbbbs, 2000,limitr, 100
      if( nbbbs.gt.2000 .or. limitr.gt.100 ) goto 920
!
...
!ik   nls = loc( dr )
!ik   nle = loc( ceqdt_emrk ) + 1
!ik   nbf = nle - nls
!ik   call MPI_Bcast( dr, nbf, MPI_BYTE, lmspe, mywld, ierr )
        call tbfind( nddt, typ_dnam, ityp, 'CEQDT' )
        itag = typ_itag(ityp)
        call MPI_Bcast( dr, 1, itag, lmspe, mywld, ierr )
      endif
      rmin = rg(1)
      rmax = rg(nr)
      zmin = zg(1)
