...
!
!-----------------------------------
!::Lz(Te) of Kr 190416 KH rlos=1e-16, ne=1e20, n0/ne=0
      zx = zte
      zx = dmin1(zx,1.0d4)
      zx = dmax1(zx,1.0d0)
      zx = dlog10(zx)
      if(zx<2.198d0)then
        zy = -36.28d0 + 12.532d0*zx    - 19.651d0 *zx**2
     >                + 24.52d0 *zx**3 - 17.19d0  *zx**4
     >                + 5.7319d0*zx**5 - 0.71454d0*zx**6
      else
        zy = 187.29d0 - 433.85d0*zx    + 352.95d0 *zx**2
     >                - 150.76d0*zx**3 + 35.751d0 *zx**4
     >                - 4.4714d0*zx**5 + 0.23079d0*zx**6
      endif
      zlz = 10.0d0**zy
