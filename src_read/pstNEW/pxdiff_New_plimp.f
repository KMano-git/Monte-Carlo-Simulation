...
!ik   integer  it, i, jmax, jw, j, ia, j1, j2, ib, nsiz
!ik   integer  ii, ic, iz, irg, nty
!ik   real*8   zfimp, zte32, zti32,  fke, fki, feq, feta
...
! deleted 2 lineiseleting unused common variables in SONIC by kamata 2021/05/31
!ik   real*8   xidt, xifc, xitm
!ik   real*8   zne, zni, zzf
...
!ik   integer :: ix
!ik   integer, dimension(ndxy) :: jno
!
! deleted 2 lines organize local variables and include files by kamata 2021/05/31
!ik   integer :: i6 = 180000 + 200
!ik   integer :: lp = 0
!
...
!::frequency, coefficient
      ...
      fki = 0.0d0
      feq = 0.0d0
! add 1 line for avoid feta = 0 error 2023/3/27
      if(mion == 0) mion = 1
...
!::debug write
!-----------------------------------------------------------------------
!x      if( lpchk.eq.1 ) then
!x      write(n6,'(/2x,"***  pxdiff  ***  time =",1pe12.3,
!x     > "   it =",i3,"  i =",i3)') it,i
!x      write(n6,'(2x,2x,"jw",3x,"j",4x,"jp",5x,"xiclh",7x,"xicl",8x,
!x     >  "xiclp",7x,"xianh",7x,"xian",8x,"xianp")')
!x      do 310 jw = 1, jmax-1
!x      j  = jcel(jw,it)
!x      jp = jcel(jw+1,it)
!x      zxicl = 1.0d0/(hwtm(j,i)/xicl(j)+hwtp(j,i)/xicl(jp))
!x      zxian = 1.0d0/(hwtm(j,i)/xian(j)+hwtp(j,i)/xian(jp))
!x      write(n6,'(2x,3i5,1p6e12.3)')
!x     >  jw,j,jp,zxicl,xicl(j),xicl(jp),zxian,xian(j),xian(jp)
!x 310  continue
!x      endif
!
      return
