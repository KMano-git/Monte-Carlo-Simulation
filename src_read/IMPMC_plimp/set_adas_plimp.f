...
!ik   call getls(dratm,"f",nfatm,tbdsn,10)
      call getls(dratm(kis),"f",nfatm,tbdsn,10,'_'//cid)
!
!::table
!
...
!ik   call atm_copy("ATOM_CXR",ip_cxr)  ! rate_cxr
      call atm_copy_x("ATOM_ION",ip_ion(kis),kis)  ! rate_ion
      call atm_copy_x("ATOM_REC",ip_rec(kis),kis)  ! rate_rec
      call atm_copy_x("ATOM_CXR",ip_cxr(kis),kis)  ! rate_cxr
...
!ik   call atm_copy("ATOM_PRC",ip_prc)  ! rad_cxr  <== No use 12/01/06
      call atm_copy_x("ATOM_PLT",ip_plt(kis),kis)  ! rad_line
      call atm_copy_x("ATOM_PRB",ip_prb(kis),kis)  ! rad_rec
      call atm_copy_x("ATOM_PRC",ip_prc(kis),kis)  ! rad_cxr  <== No use 12/01/06
...
!ik  >  ip_ion, ip_rec, ip_cxr, ip_plt, ip_prb, ip_prc
     >  ip_ion(kis), ip_rec(kis), ip_cxr(kis), ip_plt(kis), ip_prb(kis)
     > ,ip_prc(kis)
      call flush(n6)
!
...
!ik  >    ip_plt.eq.0 .or. ip_prb.eq.0 ) goto 910
      if( ip_ion(kis) == 0 .or. ip_rec(kis) == 0 .or.
     >    ip_plt(kis) == 0 .or. ip_prb(kis) == 0 ) goto 910
!
