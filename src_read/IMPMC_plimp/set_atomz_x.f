!***********************************************************************
      subroutine set_atomz_x(drnm,kis)
!***********************************************************************
!
...
!::dratm
      dratm(kis) = trim(drnm)
!
      m1 = index( dratm(kis), "/", .true. )
      m2 = index( dratm(kis), "_", .true. )
      catmz(kis) = dratm(kis)(m1+1:m2-1)
!
      write(n6,'(2x,"catmz = ",a,"  dratm = ",a)')
     >     catmz(kis), trim(dratm(kis))
      call flush(n6)
!
!
!::atomic data
...
!::atom to be treated in this group PE
      ityp = 0
      if( trim(catmz(kis)).eq."C"  ) ityp = 1
      if( trim(catmz(kis)).eq."Ne" ) ityp = 2
      if( trim(catmz(kis)).eq."Ar" ) ityp = 3
      if( trim(catmz(kis)).eq."Kr" ) ityp = 4
      if( trim(catmz(kis)).eq."Xe" ) ityp = 5
      if( trim(catmz(kis)).eq."He" ) ityp = 6
      if( trim(catmz(kis)).eq."Be" ) ityp = 7
      if( trim(catmz(kis)).eq."W"  ) ityp = 8
      if( trim(catmz(kis)).eq."N"  ) ityp = 9
      if( ityp.eq.0 ) then
      write(n6,'(2x,"No found catmz in tbcatmz  ",a)') trim(catmz(kis))
      mstop = 1
      return
      endif
!
!::common variabels in catcom
      catmz(kis) = tbcatmz(ityp)
      natmz(kis) = min(tbnatmz(ityp), ndmis)
      aionz(kis) = tbaionz(ityp)
!
...
!
      eipot(0:ndmis) = 0.0d0
      if( ityp.eq.1 ) eipot(0:natmz(kis)) = tbeipot_C(0:natmz(kis))
      if( ityp.eq.2 ) eipot(0:natmz(kis)) = tbeipot_Ne(0:natmz(kis))
      if( ityp.eq.3 ) eipot(0:natmz(kis)) = tbeipot_Ar(0:natmz(kis))
      if( ityp.eq.4 ) eipot(0:natmz(kis)) = tbeipot_Kr(0:natmz(kis))
      if( ityp.eq.5 ) eipot(0:natmz(kis)) = tbeipot_Xe(0:natmz(kis))
      if( ityp.eq.6 ) eipot(0:natmz(kis)) = tbeipot_He(0:natmz(kis))
      if( ityp.eq.7 ) eipot(0:natmz(kis)) = tbeipot_Be(0:natmz(kis))
      if( ityp.eq.8 ) eipot(0:natmz(kis)) = tbeipot_W(0:natmz(kis))
      if( ityp.eq.9 ) eipot(0:natmz(kis)) = tbeipot_N(0:natmz(kis))
!
      write(n6,'(2x,"catmz = ",a,"  natmz =",i3,"  aionz =",f8.3)')
     >   trim(catmz(kis)), natmz(kis), aionz(kis)
      write(n6,'(2x,"eipot = ",10f10.3)') (eipot(iz),iz=0,natmz(kis))
      call flush(n6)

      if( eipot(kis).eq.0.0d0 ) then
