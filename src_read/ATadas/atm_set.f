! deleted atm_fnam_2, atm_fnam_3 treat 4 or more impurities with IMPMC by kamata 2022/04/15
! 2018.10.23
! This code is a corrected version of atm_set.f.
! This correction was made to avoid problems often encountered on JFRS-1.
! There was some problem between MPI library for some version and libc.
! We use libc without fortran wrapper.
!
!x      call test_atm_set
!x      stop
!x      end
!
!***********************************************************************
      subroutine test_atm_set
!***********************************************************************
      use cunit, only : n6
      implicit none
      character(2)  :: catm
      character(80) :: cdir, dsn
!
      n6 = 6
      write(n6,'(2x,a)') "setenv drads ~/sonicV2/adas2/Ne_opt2"
!
      call getenv("drads", cdir)
      write(n6,'(2x,"setenv drads  cdir = ",a)') trim(cdir)
!
      call atm_set(cdir,catm)
!
      call atm_fnam("ATOM_ION",dsn)
      call atm_fnam("ATOM_REC",dsn)
      call atm_fnam("ATOM_CXR",dsn)
      call atm_fnam("ATOM_PLT",dsn)
!
      call atm_fnam("ATOM_RAD",dsn)
      call atm_fnam("ATOM_PRB",dsn)
      call atm_fnam("ATOM_PRC",dsn)
!
      return
      end
!
!***********************************************************************
      subroutine atm_set(drnm,catm)
!***********************************************************************
!
