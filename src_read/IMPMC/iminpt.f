...
!-----------------------------------------------------------------------
     ...
     >    , use_gaspufftime_IMP, pufftime_IMP, pf_mag_time, pf_rate_time
     >    , pufftime_max_IMP, pufftime_size_IMP
     ...
     >    , void_ne, void_ni, void_te, void_ti
      ...
      use cplcom, only: lerp_points_vldar,lerp_points_vletr
     >  ,lerp_points_vlxir,lerp_points_vlxer
     >  ,lerp_r_vldar,lerp_diff_vldar,lerp_r_vletr,lerp_diff_vletr
     >  ,lerp_r_vlxir,lerp_diff_vlxir,lerp_r_vlxer,lerp_diff_vlxer
     >  ,vlda, cftr
      use mod_externalgrid, only : use_exdata
     >  ,vac_ele_size, pri_ele_size
      implicit none
!
!::argument
...
!::local variables
      ...
      integer nsmp_total, ierr
      real(8), parameter :: pump_ref_def = 0.98d0

! for uwlinp
      real*8  rcy_wl, rcy_vs, rcy_di, rcy_do, rcy_ws
     >       ,tem_wl, tem_vs, tem_di, tem_do
     >       ,rcy_gt(4), rcy_al(4), rcy_pa(4), rcy_sh(4)
     >       ,tem_gt(4), tem_al(4), tem_pa(4), tem_sh(4)
      integer :: lrf_gt(4) = 0, lrf_vs = 0
      integer :: lrf_sh(4) = 0, lrf_wl = 0
      logical lex
      integer mj
      character(80) :: rNumber_st
...
!::time depending puff rate setting
      namelist /gaspuff_IMP_time/
     > use_gaspufftime_IMP, pufftime_IMP, pf_mag_time, pf_rate_time

!::for over write reflection coeffient by neut2d value
      namelist/uwlinp/ 
...
!
      call set_atomz(cdatm)
      write(n6,'(/2x,"impurity : ",a2,"  ismax =",i3,"  azmas =",
     >   f10.3)') catmz(1), ismax, azmas
!
      ami = aimas*cmp
      amz = azmas*cmp
      if(use_exdata) then
        irmax = ncmax+vac_ele_size+pri_ele_size
      else
        irmax = ncmax
      endif
!xx   v0inZ = sqrt(2.0d0*e0inZ*cev/amz)
      lpmax = ltmax*ltout
...
!
!:: diffsion coefficient for IMPMC
      vlda = -99.0d0 ! initialiation for avoid unintended value.
      call read_upldif(nft,ierr)
      if(match_solimpdiff) then
        do i = 1, mrgn_max
          cdifrg_imp(i,nprg) = vlda(i)
        enddo
        lerp_r_cdif    = 0.0_8
        lerp_diff_cdif = 0.0_8
        lerp_points_cdif(nprg) = lerp_points_vldar
        do i = 1, lerp_points_vldar
          lerp_r_cdif(i,nprg) = lerp_r_vldar(i)
          lerp_diff_cdif(i,nprg) = lerp_diff_vldar(i)
        enddo
      endif
      do i = 1, mrgn_max
        if(cdifrg_imp(i,nprg) .ge. 0.0d0) then
          cdifrg(i) = cdifrg_imp(i,nprg)
        endif
      enddo
      if(cdif_imp(nprg)>0.0d0) cdif = cdif_imp(nprg)

!::time depending puff rate setting
      rewind (nft)
      read(nft,nml=gaspuff_IMP_time,iostat=ierr)
      rewind (nft)
      if(use_gaspufftime_IMP) then
        call replace_pfmag_pfrate
        call check_t_order(pufftime_IMP,pufftime_max_IMP
     >   ,pufftime_size_IMP)
        write(n6,'(5x,"time depending impurity puff-rate is used")')
      endif

!::set IMPMC refrection cefficient(rflpmp) as that of neut2d(rcy_al)
      if(use_neut_refl) then
        rcy_al = pump_ref_def
        read(nft,uwlinp)
        rflpmp(1:4) = rcy_al(1:4)
      endif
!
...
!
!::set restart file
      write(n6,'(2x," nftwZ, nftrZ, lcalZ, ndskZ in inppls is unused")')
      write(n6,'(2x," cftwZ in inppls is ignored")')
!
      if( impmc_model == 1 ) then
      !:: set read restart file as consistent with soldor(cftr=../dtprf_XX)
        inquire(file=trim(cftr), exist=lex)
        if(lex) then
          if( cftrZ == cftrZ_def ) then
            ! set default name if it is not overwriten by inppls
            cftrZ = "RSTMC-"//trim(catmz(1))//trim(grnm(mygrp))
          endif
          mj = index(cftr,"_")
          rNumber_st = trim(cftr(mj+1:))
          cftrZ = "../wxdr_"//trim(rNumber_st)//"/"//cftrZ
        else
          cftrZ = " "
        endif
      !:: set write restart file
        cftwZ= "../"//trim(wh_dir)//"/RSTMC-"
     >    //trim(catmz(1))//trim(grnm(mygrp))
!
        write(n6,'(2x," read-rst file  cftrZ = ",a)') trim(cftrZ)
        write(n6,'(2x,"write-rst file  cftwZ = ",a)') trim(cftwZ)

!::imp_cutbk
        write(n6,'(2x,"lcutz =",i3,"  nhr/ndhr =",2i5,"  jrmax =",i4,
     >    "  npcut =",i6,"  nhsiz =",i5)') lcutz, nhr, ndhr, jrmax,
     >    npcut, nhsiz
        write(n6,'(2x,"tmcut =",f8.3,"  dtcut =",f8.3,"  nctst =",i6)')
     >    tmcut, dtcut, nctst
      endif
...
!**********************************************************************
      contains
      subroutine replace_pfmag_pfrate
!**********************************************************************
      use cimcom, only:azmas, aimas
      use cimpuf, only:ndpfz, pufftime_max_IMP, pf_rate_time
     > ,  pf_mag_time
...
!::local variables
      ...
      pfrt = 0.24d21 ! this coefficient comes from the ideal gas equation
      do i = 1, ndpfz
        do i_time = 1, pufftime_max_IMP
          if(pf_rate_time(i,i_time,nprg).ne.0.0d0) then
            pf_mag_time(i,:,nprg) = pfrt*pf_rate_time(i,:,nprg)
            goto 100
          endif
        enddo
 100    continue
      enddo
      write(n6,'(/2x,"pfrt=",1pe14.6)') pfrt
      end subroutine replace_pfmag_pfrate

!***********************************************************************
      subroutine imsave_allocate
!***********************************************************************
      use csize,  only : ndmis, ndmc
      use cimcom, only: sflux_save, swtot_save, sptyc_save
     ...
     > , stimp_save, sdtmz_save, sdmy1_save, sdmy2_save, sdmy3_save
     > , hcnpt_save, sitmp_save
...
! local variables
      ...
      if( .not. allocated( sflux_save ) ) then
        allocate( sflux_save(nsput),swtot_save(nsput)
     >      , sptyc_save(nsput), stimz_save(nsput)
     >      , denZ_save(0:ndmis,0:ndmc,nsput)
     >      , temZ_save(0:ndmis,0:ndmc,nsput)
     >      , wsct_save(0:ndmc,nsput)
     >      , hrgpc_save(0:10,nsput)
     >      , phwrd_save(ndmc,nsput)
     >      , phwci_save(ndmc,nsput)
     >      , phdnz_save(0:ndmis,ndmc,nsput)
     >      , phvlz_save(0:ndmis,ndmc,nsput)
     >      , hgrpp_save(15,nsput), hgrpw_save(15,nsput)
     >      , hregp_save(10,nsput), hregw_save(10,nsput)
     >      , friZ_save(0:ndmis,0:ndmc,nsput)
     >      , thfZ_save(0:ndmis,0:ndmc,nsput)
     >      , vzpZ_save(0:ndmis,0:ndmc,nsput)
     >      , ionZ_save(0:ndmis,0:ndmc,nsput)
     >      , recZ_save(0:ndmis,0:ndmc,nsput)
     >      , phfrz_save(0:ndmis,ndmc,nsput)
     >      , phthz_save(0:ndmis,ndmc,nsput)
     >      , phionz_save(0:ndmis,ndmc,nsput)
     >      , phrecz_save(0:ndmis,ndmc,nsput)
     >      , hcnwt_save(0:10,nsput)
     >      , hrgpa_save(0:10,nsput)
     >      , sptcl_save(nsput), stimp_save(nsput), sdtmz_save(nsput)
     >      , sdmy1_save(nsput), sdmy2_save(nsput), sdmy3_save(nsput)
     >      , hcnpt_save(0:10,nsput), sitmp_save(nsput)
     >      , stat = istat )
        if( istat /= 0 ) then
          write(cmsg,'(a,i6)')
     >        'sflux_save allocate error in imsave_allocate
     >         , istat = ', istat
          call wexit( 'imsave_allocate', trim( cmsg ) )
        endif
      endif
      end subroutine imsave_allocate

      end subroutine iminpt
