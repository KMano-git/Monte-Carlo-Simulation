...
module cdf_reader
   ...
   integer, parameter :: N_ENERGY_SIGMA = 101
   integer, parameter :: N_ENERGY_ANGLE = 51
   integer, parameter :: N_TEMP_GRID    = 51
   integer, parameter :: N_PROB         = 251

   real(dp) :: energy_grid_sigma(N_ENERGY_SIGMA)
   real(dp) :: sigma_elastic(N_ENERGY_SIGMA)
   real(dp) :: energy_grid_angle(N_ENERGY_ANGLE)
   real(dp) :: temp_grid(N_TEMP_GRID)
   real(dp) :: prob_grid(N_PROB)
...
contains
   ...
   subroutine read_named_real_array(filename, var_name, values, n_read, ierr)
      ...
      in_data = .false.
      iunit = 21
      allocate(character(len=1000000) :: buffer)
      ...
      do while (in_data)
         ...
         do while (buf_pos <= buf_len)
            ...
            if (buf_pos > start_pos) then
               ...
               if (n_read < size(values)) then
                  n_read = n_read + 1
                  read(token, *, iostat=ios) values(n_read)
                  if (ios /= 0) n_read = n_read - 1
               end if
            end if
         end do
   ...
   function get_sigma_elastic(energy) result(sigma)
      ...
      t = (log_E - (log_E_grid_sigma_min + (idx - 1) * log_E_grid_sigma_dlt)) / &
         log_E_grid_sigma_dlt
   ...
   function sample_scattering_angle(energy, rand_p) result(chi)
      ...
      t = (log_E - (log_E_grid_angle_min + (e_idx - 1) * log_E_grid_angle_dlt)) / &
         log_E_grid_angle_dlt
      t = max(0.0d0, min(1.0d0, t))
   ...
   function interp_2d(val_table, energy, temp) result(val)
      ...
      e_idx = int((log_E - log_E_grid_angle_min) / log_E_grid_angle_dlt) + 1
      e_idx = max(1, min(e_idx, N_ENERGY_ANGLE - 1))
      t_E = (log_E - (log_E_grid_angle_min + (e_idx - 1) * log_E_grid_angle_dlt)) / &
         log_E_grid_angle_dlt
