...
# usage: cd 3d3v_natl/ && gnuplot plot_histogram.gp
#===============================================================================
...
# データ行数を取得
n_rows = int(system("awk -F',' 'NR>1 && NF>5{n++} END{print n+0}' run/energy_hist.csv"))

# 各行のステップ番号を取得
system("awk -F',' 'NR>1 && NF>5 { gsub(/ /,\"\",$1); print $1 }' run/energy_hist.csv > /tmp/hist_steps.dat")
...
set multiplot layout 2,3 title '3D3V Non-Analog Monte Carlo: Energy Distribution Evolution' font ',14'

do for [row=0:n_rows-1] {
    stitle = system(sprintf("sed -n '%dp' /tmp/hist_steps.dat", row+1))
...
# 一時ファイル削除
system("rm -f /tmp/hist_row_*.dat /tmp/hist_steps.dat")
