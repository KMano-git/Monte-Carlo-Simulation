...
module data_types
   ...
   type :: score_data
      ...
      real(dp) :: tl_lookup_ei    !電離寄与 (テーブルルックアップ)
      real(dp) :: tl_lookup_cx    !荷電交換寄与 (テーブルルックアップ)
      real(dp) :: tl_lookup_el    !弾性散乱寄与 (テーブルルックアップ)
      ! Elastic-only comparison add-ons [J]
      real(dp) :: tl_inner_el     !inner multi-sampling TL
      real(dp) :: tl_pretab_el    !pre-tabulated TL
   end type score_data
