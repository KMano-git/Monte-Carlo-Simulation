...
contains
   ...
   !-----------------------------------------------------------------------------
   ! 半Maxwell分布から vx > 0 の速度をサンプリング
   ! 入射粒子用: f(vx) ∝ vx * exp(-vx^2 / (2*v_th^2)) for vx > 0
   ! → vx = v_th * sqrt(-log(r))  (rは一様乱数)
   ! vy, vz は通常のMaxwell分布
   !-----------------------------------------------------------------------------
   subroutine sample_half_maxwell_velocity(T_eV, vx, vy, vz)
      real(dp), intent(in)  :: T_eV  ! 温度 [eV]
      real(dp), intent(out) :: vx, vy, vz

      real(dp) :: v_th, r1, r3, r4, r5, r6

      ! 熱速度
      v_th = sqrt(T_eV * EV_TO_J / M_D_kg)

      ! vx: 半Maxwell分布 (flux-weighted)
      ! f(vx) ∝ vx * exp(-m*vx^2/(2kT)) → vx = v_th * sqrt(-2*log(r))
      call random_number(r1)
      vx = v_th * sqrt(-2.0d0 * log(max(r1, 1.0d-30)))
      ...
      vy = v_th * sqrt(-2.0d0 * log(max(r3, 1.0d-30))) * cos(2.0d0 * PI * r4)
      vz = v_th * sqrt(-2.0d0 * log(max(r5, 1.0d-30))) * cos(2.0d0 * PI * r6)
   end subroutine sample_half_maxwell_velocity
