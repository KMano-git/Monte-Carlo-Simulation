...
program monte_carlo_3d3v_natl
   use constants
   use data_types
   use random_utils, only: init_random_seed, random_double, init_rng
   ...
   real(dp) :: s_tl_cx, s_tl_el, s_tl_ei, s_tl_ei_e, s_tl_ei_n
   ...
   ! プラズマ温度用変数
   real(dp) :: E_th_old, E_th_new, delta_E_vol, delta_n_vol
   real(dp) :: n_vol_new
   real(dp) :: inv_norm_plasma
   integer  :: unit_temp
   !時間計測用
   ...
   score%tl_ei_e = 0.0d0; score%tl_ei_n = 0.0d0
   ...
   !プラズマ温度記録用ファイルの初期化
   unit_temp = 60
   open(unit=unit_temp, file='plasma_temp.csv', status='replace')
   write(unit_temp,'(A)') 'time[s],T[eV],n_e[m^-3]'
   write(unit_temp,'(ES14.6,A,ES14.6,A,ES14.6)') 0.0d0, ',', plasma%T_e, ',', plasma%n_e
   close(unit_temp)
   ...
   do istep = 1, sim%n_steps
      ...
      do ip = 1, sim%n_particles
         ...
         s_tl_ei_e = s_tl_ei_e + my_score%tl_ei_e
         s_tl_ei_n = s_tl_ei_n + my_score%tl_ei_n
      ...
      step_score%tl_ei_e = s_tl_ei_e
      step_score%tl_ei_n = s_tl_ei_n
      ...
      score%tl_ei_e = score%tl_ei_e + step_score%tl_ei_e
      score%tl_ei_n = score%tl_ei_n + step_score%tl_ei_n
      ...
      if (mod(istep, diag%output_interval) == 0) then
         write(*,'(A,I6,A,I6,A,I8,A,I6,A,I6,A,F10.4)') &
            '  Step ', istep, '/', sim%n_steps, &
            '  alive=', n_alive, &
            '  CX=', n_coll_cx, '  EL=', n_coll_el, '  T=', plasma%T_e
      end if

      !--- プラズマ温度と密度の更新 (T_i = T_e の仮定) ---
      ! 1. 今の全熱エネルギー密度 E_th [J/m³]
      E_th_old = 1.5d0 * (plasma%n_i * plasma%T_i + plasma%n_e * plasma%T_e) * EV

      ! 2. 1ステップ間のMC粒子からプラズマへのエネルギー移動量 ΔE_vol [J/m³]
      inv_norm_plasma = init_p%n_init / dble(sim%n_particles)
      delta_E_vol = (step_score%tl_ei + step_score%tl_cx + step_score%tl_el + step_score%tl_ei_e) * inv_norm_plasma

      ! 3. 1ステップ間の電離生���数密度 Δn_vol [m⁻³]
      delta_n_vol = step_score%tl_ei_n * inv_norm_plasma

      ! 4. 更新後の密度 n_i' = n_e', 全熱エネルギー密度 E_th'
      plasma%n_i = plasma%n_i + delta_n_vol
      plasma%n_e = plasma%n_e + delta_n_vol
      E_th_new = E_th_old + delta_E_vol
      n_vol_new = plasma%n_i + plasma%n_e

      ! 5. 新しい温度 T' [eV] を算出し、代入
      if (n_vol_new > 1.0d-30) then
         plasma%T_i = E_th_new / (1.5d0 * n_vol_new * EV)
         plasma%T_i = max(plasma%T_i, 1.0d-4)
         plasma%T_e = plasma%T_i
      end if

      ! 6. plasma_temp.csv への出力
      open(unit=unit_temp, file='plasma_temp.csv', status='old', position='append')
      write(unit_temp,'(ES14.6,A,ES14.6,A,ES14.6)') &
         dble(istep) * sim%dt, ',', plasma%T_e, ',', plasma%n_e
      close(unit_temp)
