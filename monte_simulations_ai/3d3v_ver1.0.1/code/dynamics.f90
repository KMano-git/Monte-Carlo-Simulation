...
contains
   ...
   !---------------------------------------------------------------------------
   ! イベント駆動用 衝突評価
   ! 仮の衝突時刻に到達した際に、それが本物のCX/EL衝突かNull衝突かを判定する
   !---------------------------------------------------------------------------
   subroutine evaluate_collision_event(p, plasma, sim, &
      vx_i, vy_i, vz_i, &
      ...
      if (r_accept > P_accept) return ! Null Collision
   ...
   ! 弾性散乱衝突: 重心系で散乱角を適用
   !---------------------------------------------------------------------------
   subroutine collision_el(p, vx_i, vy_i, vz_i, delta_E)
      type(particle_t), intent(inout) :: p
      real(dp), intent(in)  :: vx_i, vy_i, vz_i
      real(dp), intent(out) :: delta_E
