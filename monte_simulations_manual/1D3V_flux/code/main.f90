...
program main
   ...
   ! 粒子初期化
   call initialize_particles(particles, sim%n_particles, init_p, sim%x_min)
   ...
   close(21)
   call output_energy_histogram(particles, sim, diag, init_p, 0)
   call output_position_histogram(particles, sim, init_p, 0)
   ...
   do istep = 1, sim%n_steps
      ...
      ! 単位体積あたり、単位時間あたりのプラズ��エネルギ���への寄与 [W/m^3]
      ...
      if (ihist <= 5) then
         if (istep == diag%hist_timing(ihist)) then
            call output_energy_histogram(particles, sim, diag, init_p, istep)
            call output_position_histogram(particles, sim, init_p, istep)
            ihist = ihist + 1
         end if
      end if
