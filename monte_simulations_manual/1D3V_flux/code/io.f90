...
contains
   ...
   ! 粒子初期化
   !-----------------------------------------------------------------------------
   subroutine initialize_particles(particles, n_particles, init_p, x_min)
   ...
   ! 中性粒子のエネルギーヒストグラム出力
   !-----------------------------------------------------------------------------
   subroutine output_energy_histogram(particles, sim, diag, init_p, istep)
      ...
      n_per_volume = dble(sim%n_particles) * init_p%n_init
   ...
   ! 粒子の位置ヒストグラム出力
   !-----------------------------------------------------------------------------
   subroutine output_position_histogram(particles, sim, init_p, istep)
      type(particle_t), intent(in) :: particles(:)
      type(sim_params), intent(in)  :: sim
      type(init_params), intent(in) :: init_p
      integer, intent(in) :: istep
      ...
      real(dp) :: x_min, x_max
      integer, allocatable :: hist(:)
      character(len=256) :: output_file = sim%output_statx
      ...
      ! ファイルオープン
      open(unit=unit_pos, file=trim(output_file), status='unknown', position='append')

      ! ヘッダー書き込み（初回のみ）
      if (istep == 0) then
         write(unit_pos, '(A)', advance='no') 'step,x_min,x_max,dx,n_per_volume'
         do i = 1, sim%n_grid
      ...
      ! データ書き込み
      write(unit_pos, '(I0,",",F0.4,",",F0.4,",",F0.4,",",ES12.4)', advance='no') &
         istep, x_min, x_max, dx, n_per_volume
      do i = 1, sim%n_grid
