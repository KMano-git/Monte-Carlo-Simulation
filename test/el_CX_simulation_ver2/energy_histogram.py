...
固定エネルギーの中性粒子が背景プラズマを飛行するときのエネルギー受け渡し量のヒストグラム表示

Usage:
    python energy_histogram.py [En] [Ti]
    
    En: 中性粒子エネルギー [eV] (デフォルト: 100.0)
...
# シミュレーションパラメータ
N_SAMPLES = 2500000      # サンプル数（ヒストグラム用に多めに取る）
...
def sample_isotropic_velocity_fixed_energy(energy, mass, n_samples):
    """
    固定エネルギーで等方的な方向分布の速度ベクトルを生成
    ...
    Returns:
        velocity: 速度ベクトル [m/s] (n_samples, 3)
    """
    # 速度の大きさ: E = 0.5 * m * v^2 → v = sqrt(2 * E / m)
    v_mag = np.sqrt(2.0 * energy * EV_TO_J / mass)
    
    # 等方的な方向をサンプリング（単位球面上の一様分布）
    # cos(theta)を[-1, 1]から一様サンプリング、phiを[0, 2π]から一様サンプリング
    ...
    velocity[:, 0] = v_mag * sin_theta * np.cos(phi)
    velocity[:, 1] = v_mag * sin_theta * np.sin(phi)
    velocity[:, 2] = v_mag * cos_theta
...
def compute_energy_transfers(E_neutral, T_ion, n_samples,
                              energy_grid, sigma_elastic,
                              prob_grid, energy_grid_angle, angle_cdf):
    """
    固定エネルギーの中性粒子と背景プラズマイオンとのエネルギー受け渡し量を計算
    
    Parameters:
        E_neutral: 中性粒子エネルギー [eV]
        T_ion: イオン温度 [eV]
        n_samples: サンプル数
        ...
    ...
    # 速度サンプリング
    # 中性粒子: 固定エネルギー、等方的方向分布
    v_n = sample_isotropic_velocity_fixed_energy(E_neutral, M_D, n_samples)
    # イオン: マクスウェル分布
    ...
    delta_E_cx = E_n - E_i  # 速度が入れ替わる（イオンへのエネルギー寄与）
    ...
    delta_E_el = E_n - E_n_new  # イオンへのエネルギー寄与
...
def main():
    # コマンドライン引数からパラメータを取得
    E_neutral = 100.0  # デフォルト中性粒子エネルギー [eV]
    T_ion = 5.0        # デフォルトイオン温度 [eV]
    
    if len(sys.argv) >= 2:
        try:
            E_neutral = float(sys.argv[1])
        except ValueError:
            print(f"Warning: Invalid En value '{sys.argv[1]}', using default {E_neutral}")
    ...
    print("=" * 60)
    print("Energy Transfer Histogram (Fixed Energy Neutral)")
    print("=" * 60)
    print(f"  Neutral energy En = {E_neutral:.2f} eV")
    ...
    hist_range = (np.percentile(all_data, 0.01), np.percentile(all_data, 99.99))
    bins = 80
    ...
    if COMBINED_PLOT:
        ...
        ax.set_xlabel(r'Energy Transfer to Ions $\Delta E$ (eV)', fontsize=18)
        ax.set_ylabel(r'Reaction Rate ($\sigma v$) (arb. units)', fontsize=18)
        ax.set_yscale('log')
        ax.set_ylim(1e-11, 1e-7)
        # ax.set_title(f'Energy Transfer Comparison: CX vs EL\nEn = {E_neutral:.2f} eV, Ti = {T_ion:.2f} eV', 
        ...
        # 保存
        output_file = f'figure/energy_histogram_combined_En{E_neutral:.1f}_Ti{T_ion:.1f}.png'
        ...
        ax1.set_yscale('log')
        ax1.set_title(f'Charge Exchange (CX)\nEn = {E_neutral:.2f} eV, Ti = {T_ion:.2f} eV', fontsize=12)
        ...
        # 保存
        output_file = f'figure/energy_histogram_En{E_neutral:.1f}_Ti{T_ion:.1f}.png'
        plt.savefig(output_file, dpi=200, bbox_inches='tight')
        print(f"\nPlot saved to: {output_file}")
